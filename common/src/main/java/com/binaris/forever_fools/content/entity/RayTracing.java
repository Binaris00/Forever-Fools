...
public class RayTracing extends PathfinderMob {
    ...
    private static final List<Component> RAYS_DEATH_LINES = Stream.of(
    ...
    private static final List<Component> RAYS_RANDOM_LINES = Stream.of(
    ...
    private static final List<Component> RAYS_INTRO_LINES = Stream.of(
    ...
    private static final List<Component> RAYS_OUTRO_LINES = Stream.of(
    ...
    public void tick() {
        if (this.nextLine <= this.level().getGameTime()) {
            ...
            if (this.firstJoin) {
                ...
                this.say(RAYS_RANDOM_LINES.get(this.random.nextInt(RAYS_RANDOM_LINES.size())));
            }

            // TODO: Raytracing using Rules mechanic, mmmmmmm
...
//                            .getPlayerList()
//                            .broadcastSystemMessage(Component.translatable("multiplayer.player.left",
//                                    "Ray Tracing").withStyle(ChatFormatting.YELLOW), false);
//                }
//            }
        }
    ...
    protected void tickDeath() {
        ...
        if (this.deathTime >= 20 && !this.level().isClientSide() && this.level() instanceof ServerLevel serverLevel) {
            // TODO: Method to add ray tracing
            //serverLevel.addRayTracing();
            this.say(RAYS_DEATH_LINES.get(this.random.nextInt(RAYS_DEATH_LINES.size())));
        }
    }
    ...
    public void sayIntro() {
        this.say(RAYS_INTRO_LINES.get(this.random.nextInt(RAYS_INTRO_LINES.size())));
    }

    public void sayOutro() {
        this.say(RAYS_OUTRO_LINES.get(this.random.nextInt(RAYS_OUTRO_LINES.size())));
    }
}