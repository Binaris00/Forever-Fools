...
public class ToxifinModel extends HierarchicalModel<ToxifinSlab> {
    public static final ModelLayerLocation LAYER_LOCATION = new ModelLayerLocation(FFCommonMod.id("toxifin"), "main");
    ...
    public void setupAnim(@NotNull ToxifinSlab toxifinSlab, float f, float g, float h, float i, float j) {
        float k = h - (float)toxifinSlab.tickCount;
        this.head.yRot = i * (float) (Math.PI / 180.0);
        this.head.xRot = j * (float) (Math.PI / 180.0);
        float l = (1.0F - toxifinSlab.getSpikesAnimation(k)) * 0.55F;
        this.setupSpikesToxic(h, l, toxifinSlab.isVehicle(), toxifinSlab.isPassenger());

        Entity entity = Minecraft.getInstance().getCameraEntity();
        if (toxifinSlab.hasActiveAttackTarget()) {
            entity = toxifinSlab.getActiveAttackTarget();
        }

        if (entity != null) {
            Vec3 vec3 = entity.getEyePosition(0.0F);
            Vec3 vec32 = toxifinSlab.getEyePosition(0.0F);
            ...
            Vec3 vec33 = toxifinSlab.getViewVector(0.0F);
        ...
        float m = toxifinSlab.getTailAnimation(k);
